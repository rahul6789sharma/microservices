...
public class Demo3 {
	...
	// [(75,76,77,79), (75,79)]
	static List<List<SubModule>> moduleReltion = new ArrayList<>();
...
/*	private static void makeRelation(List<List<Integer>> lst) {
	...
	public static void main(String[] args) {
		...
		getModulesChildList(subModules, subModules1);
	}

	private static List<List<SubModule>> getModulesChildList(List<SubModule> subModuleList, SubModule id) {
		List<List<SubModule>> result = new ArrayList<>();
		List<SubModule> mainModules = new ArrayList<>();
		List<SubModule> subModules = new ArrayList<>();
		for (SubModule item : subModuleList) {

			if (item.getModID() == id.getModID()) {
		...
		for (List<SubModule> item : result) {
			List<SubModule> childs = new ArrayList<>();
			childs.add(id);
			getChild(item, id.getModID(), childs);
	...
	private static Integer getChild(List<SubModule> subModules, Integer parent, List<SubModule> childs) {

		for (SubModule subModule : subModules) {
			if (subModule.getModID() == parent) {
				// System.out.println(parent + " ->" + subModule.getSubModID());
				childs.add(subModule);
