			...
			try {
				...
				while (!status && retryCounter < RETRY) {
					...
					try {
						...
						SendEmail.sentMail("CRITICAL! Retry Bhav Copy is not downloaded",
				...
				SendEmail.sentMail("OI Bhav Copy is downloaded",
						"url " + url + "\n completeFileName" + completeFileName, "Data-Downloader");
				if (status) {
					...
					SendEmail.sentMail("CRITICAL! Bhav Copy is not downloaded",
							"url " + url + "\n completeFileName" + completeFileName, "Data-Downloader");
				}
			} catch (Exception e) {
				e.printStackTrace();
