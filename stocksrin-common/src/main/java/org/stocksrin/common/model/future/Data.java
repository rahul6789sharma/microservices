package org.stocksrin.common.model.future;

public class Data {
	
	private String annualisedVolatility;
	private String bestBuy;
	private String totalSellQuantity;
	private String vwap;
	private String clientWisePositionLimits;
	private String optionType;
	private String highPrice;
	private String dailyVolatility;
	private String bestSell;
	private String marketLot;
	private String sellQuantity5;
	private String marketWidePositionLimits;
	private String sellQuantity4;
	private String sellQuantity3;
	private String sellQuantity2;
	private String underlying;
	private String sellQuantity1;
	private String pChange;
	private String premiumTurnover;
	private String totalBuyQuantity;
	private String turnoverinRsLakhs;
	private String changeinOpenInterest;
	private String strikePrice;
	private String openInterest;
	private String buyPrice2;
	private String buyPrice1;
	private String openPrice;
	private String prevClose;
	private String expiryDate;
	private String lowPrice;
	private String buyPrice4;
	private String buyPrice3;
	private String buyPrice5;
	private String numberOfContractsTraded;
	private String instrumentType;
	private String sellPrice1;
	private String sellPrice2;
	private String sellPrice3;
	private String sellPrice4;
	private String sellPrice5;
	private String change;
	private String pchangeinOpenInterest;
	private String ltp;
	private String impliedVolatility;
	private String underlyingValue;
	private String buyQuantity4;
	private String buyQuantity3;
	private String buyQuantity2;
	private String buyQuantity1;
	private String buyQuantity5;
	private String settlementPrice;
	private String closePrice;
	private String lastPrice;

	public String getAnnualisedVolatility() {
		return annualisedVolatility;
	}

	public void setAnnualisedVolatility(String annualisedVolatility) {
		this.annualisedVolatility = annualisedVolatility;
	}

	public String getBestBuy() {
		return bestBuy;
	}

	public void setBestBuy(String bestBuy) {
		this.bestBuy = bestBuy;
	}

	public String getTotalSellQuantity() {
		return totalSellQuantity;
	}

	public void setTotalSellQuantity(String totalSellQuantity) {
		this.totalSellQuantity = totalSellQuantity;
	}

	public String getVwap() {
		return vwap;
	}

	public void setVwap(String vwap) {
		this.vwap = vwap;
	}

	public String getClientWisePositionLimits() {
		return clientWisePositionLimits;
	}

	public void setClientWisePositionLimits(String clientWisePositionLimits) {
		this.clientWisePositionLimits = clientWisePositionLimits;
	}

	public String getOptionType() {
		return optionType;
	}

	public void setOptionType(String optionType) {
		this.optionType = optionType;
	}

	public String getHighPrice() {
		return highPrice;
	}

	public void setHighPrice(String highPrice) {
		this.highPrice = highPrice;
	}

	public String getDailyVolatility() {
		return dailyVolatility;
	}

	public void setDailyVolatility(String dailyVolatility) {
		this.dailyVolatility = dailyVolatility;
	}

	public String getBestSell() {
		return bestSell;
	}

	public void setBestSell(String bestSell) {
		this.bestSell = bestSell;
	}

	public String getMarketLot() {
		return marketLot;
	}

	public void setMarketLot(String marketLot) {
		this.marketLot = marketLot;
	}

	public String getSellQuantity5() {
		return sellQuantity5;
	}

	public void setSellQuantity5(String sellQuantity5) {
		this.sellQuantity5 = sellQuantity5;
	}

	public String getMarketWidePositionLimits() {
		return marketWidePositionLimits;
	}

	public void setMarketWidePositionLimits(String marketWidePositionLimits) {
		this.marketWidePositionLimits = marketWidePositionLimits;
	}

	public String getSellQuantity4() {
		return sellQuantity4;
	}

	public void setSellQuantity4(String sellQuantity4) {
		this.sellQuantity4 = sellQuantity4;
	}

	public String getSellQuantity3() {
		return sellQuantity3;
	}

	public void setSellQuantity3(String sellQuantity3) {
		this.sellQuantity3 = sellQuantity3;
	}

	public String getSellQuantity2() {
		return sellQuantity2;
	}

	public void setSellQuantity2(String sellQuantity2) {
		this.sellQuantity2 = sellQuantity2;
	}

	public String getUnderlying() {
		return underlying;
	}

	public void setUnderlying(String underlying) {
		this.underlying = underlying;
	}

	public String getSellQuantity1() {
		return sellQuantity1;
	}

	public void setSellQuantity1(String sellQuantity1) {
		this.sellQuantity1 = sellQuantity1;
	}

	public String getPChange() {
		return pChange;
	}

	public void setPChange(String pChange) {
		this.pChange = pChange;
	}

	public String getPremiumTurnover() {
		return premiumTurnover;
	}

	public void setPremiumTurnover(String premiumTurnover) {
		this.premiumTurnover = premiumTurnover;
	}

	public String getTotalBuyQuantity() {
		return totalBuyQuantity;
	}

	public void setTotalBuyQuantity(String totalBuyQuantity) {
		this.totalBuyQuantity = totalBuyQuantity;
	}

	public String getTurnoverinRsLakhs() {
		return turnoverinRsLakhs;
	}

	public void setTurnoverinRsLakhs(String turnoverinRsLakhs) {
		this.turnoverinRsLakhs = turnoverinRsLakhs;
	}

	public String getChangeinOpenInterest() {
		return changeinOpenInterest;
	}

	public void setChangeinOpenInterest(String changeinOpenInterest) {
		this.changeinOpenInterest = changeinOpenInterest;
	}

	public String getStrikePrice() {
		return strikePrice;
	}

	public void setStrikePrice(String strikePrice) {
		this.strikePrice = strikePrice;
	}

	public String getOpenInterest() {
		return openInterest;
	}

	public void setOpenInterest(String openInterest) {
		this.openInterest = openInterest;
	}

	public String getBuyPrice2() {
		return buyPrice2;
	}

	public void setBuyPrice2(String buyPrice2) {
		this.buyPrice2 = buyPrice2;
	}

	public String getBuyPrice1() {
		return buyPrice1;
	}

	public void setBuyPrice1(String buyPrice1) {
		this.buyPrice1 = buyPrice1;
	}

	public String getOpenPrice() {
		return openPrice;
	}

	public void setOpenPrice(String openPrice) {
		this.openPrice = openPrice;
	}

	public String getPrevClose() {
		return prevClose;
	}

	public void setPrevClose(String prevClose) {
		this.prevClose = prevClose;
	}

	public String getExpiryDate() {
		return expiryDate;
	}

	public void setExpiryDate(String expiryDate) {
		this.expiryDate = expiryDate;
	}

	public String getLowPrice() {
		return lowPrice;
	}

	public void setLowPrice(String lowPrice) {
		this.lowPrice = lowPrice;
	}

	public String getBuyPrice4() {
		return buyPrice4;
	}

	public void setBuyPrice4(String buyPrice4) {
		this.buyPrice4 = buyPrice4;
	}

	public String getBuyPrice3() {
		return buyPrice3;
	}

	public void setBuyPrice3(String buyPrice3) {
		this.buyPrice3 = buyPrice3;
	}

	public String getBuyPrice5() {
		return buyPrice5;
	}

	public void setBuyPrice5(String buyPrice5) {
		this.buyPrice5 = buyPrice5;
	}

	public String getNumberOfContractsTraded() {
		return numberOfContractsTraded;
	}

	public void setNumberOfContractsTraded(String numberOfContractsTraded) {
		this.numberOfContractsTraded = numberOfContractsTraded;
	}

	public String getInstrumentType() {
		return instrumentType;
	}

	public void setInstrumentType(String instrumentType) {
		this.instrumentType = instrumentType;
	}

	public String getSellPrice1() {
		return sellPrice1;
	}

	public void setSellPrice1(String sellPrice1) {
		this.sellPrice1 = sellPrice1;
	}

	public String getSellPrice2() {
		return sellPrice2;
	}

	public void setSellPrice2(String sellPrice2) {
		this.sellPrice2 = sellPrice2;
	}

	public String getSellPrice3() {
		return sellPrice3;
	}

	public void setSellPrice3(String sellPrice3) {
		this.sellPrice3 = sellPrice3;
	}

	public String getSellPrice4() {
		return sellPrice4;
	}

	public void setSellPrice4(String sellPrice4) {
		this.sellPrice4 = sellPrice4;
	}

	public String getSellPrice5() {
		return sellPrice5;
	}

	public void setSellPrice5(String sellPrice5) {
		this.sellPrice5 = sellPrice5;
	}

	public String getChange() {
		return change;
	}

	public void setChange(String change) {
		this.change = change;
	}

	public String getPchangeinOpenInterest() {
		return pchangeinOpenInterest;
	}

	public void setPchangeinOpenInterest(String pchangeinOpenInterest) {
		this.pchangeinOpenInterest = pchangeinOpenInterest;
	}

	public String getLtp() {
		return ltp;
	}

	public void setLtp(String ltp) {
		this.ltp = ltp;
	}

	public String getImpliedVolatility() {
		return impliedVolatility;
	}

	public void setImpliedVolatility(String impliedVolatility) {
		this.impliedVolatility = impliedVolatility;
	}

	public String getUnderlyingValue() {
		return underlyingValue;
	}

	public void setUnderlyingValue(String underlyingValue) {
		this.underlyingValue = underlyingValue;
	}

	public String getBuyQuantity4() {
		return buyQuantity4;
	}

	public void setBuyQuantity4(String buyQuantity4) {
		this.buyQuantity4 = buyQuantity4;
	}

	public String getBuyQuantity3() {
		return buyQuantity3;
	}

	public void setBuyQuantity3(String buyQuantity3) {
		this.buyQuantity3 = buyQuantity3;
	}

	public String getBuyQuantity2() {
		return buyQuantity2;
	}

	public void setBuyQuantity2(String buyQuantity2) {
		this.buyQuantity2 = buyQuantity2;
	}

	public String getBuyQuantity1() {
		return buyQuantity1;
	}

	public void setBuyQuantity1(String buyQuantity1) {
		this.buyQuantity1 = buyQuantity1;
	}

	public String getBuyQuantity5() {
		return buyQuantity5;
	}

	public void setBuyQuantity5(String buyQuantity5) {
		this.buyQuantity5 = buyQuantity5;
	}

	public String getSettlementPrice() {
		return settlementPrice;
	}

	public void setSettlementPrice(String settlementPrice) {
		this.settlementPrice = settlementPrice;
	}

	public String getClosePrice() {
		return closePrice;
	}

	public void setClosePrice(String closePrice) {
		this.closePrice = closePrice;
	}

	public String getLastPrice() {
		return lastPrice;
	}

	public void setLastPrice(String lastPrice) {
		this.lastPrice = lastPrice;
	}

	@Override
	public String toString() {
		return "ClassPojo [annualisedVolatility = " + annualisedVolatility + ", bestBuy = " + bestBuy + ", totalSellQuantity = " + totalSellQuantity + ", vwap = " + vwap
				+ ", clientWisePositionLimits = " + clientWisePositionLimits + ", optionType = " + optionType + ", highPrice = " + highPrice + ", dailyVolatility = " + dailyVolatility
				+ ", bestSell = " + bestSell + ", marketLot = " + marketLot + ", sellQuantity5 = " + sellQuantity5 + ", marketWidePositionLimits = " + marketWidePositionLimits + ", sellQuantity4 = "
				+ sellQuantity4 + ", sellQuantity3 = " + sellQuantity3 + ", sellQuantity2 = " + sellQuantity2 + ", underlying = " + underlying + ", sellQuantity1 = " + sellQuantity1 + ", pChange = "
				+ pChange + ", premiumTurnover = " + premiumTurnover + ", totalBuyQuantity = " + totalBuyQuantity + ", turnoverinRsLakhs = " + turnoverinRsLakhs + ", changeinOpenInterest = "
				+ changeinOpenInterest + ", strikePrice = " + strikePrice + ", openInterest = " + openInterest + ", buyPrice2 = " + buyPrice2 + ", buyPrice1 = " + buyPrice1 + ", openPrice = "
				+ openPrice + ", prevClose = " + prevClose + ", expiryDate = " + expiryDate + ", lowPrice = " + lowPrice + ", buyPrice4 = " + buyPrice4 + ", buyPrice3 = " + buyPrice3
				+ ", buyPrice5 = " + buyPrice5 + ", numberOfContractsTraded = " + numberOfContractsTraded + ", instrumentType = " + instrumentType + ", sellPrice1 = " + sellPrice1 + ", sellPrice2 = "
				+ sellPrice2 + ", sellPrice3 = " + sellPrice3 + ", sellPrice4 = " + sellPrice4 + ", sellPrice5 = " + sellPrice5 + ", change = " + change + ", pchangeinOpenInterest = "
				+ pchangeinOpenInterest + ", ltp = " + ltp + ", impliedVolatility = " + impliedVolatility + ", underlyingValue = " + underlyingValue + ", buyQuantity4 = " + buyQuantity4
				+ ", buyQuantity3 = " + buyQuantity3 + ", buyQuantity2 = " + buyQuantity2 + ", buyQuantity1 = " + buyQuantity1 + ", buyQuantity5 = " + buyQuantity5 + ", settlementPrice = "
				+ settlementPrice + ", closePrice = " + closePrice + ", lastPrice = " + lastPrice + "]";
	}
}
