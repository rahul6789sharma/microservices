...
public class StrategyPrinterConsoleUSDINR {

	// private static final Logger log =
	// LoggerFactory.getLogger(StrategyPrinterConsole.class);
	private static DecimalFormat strike = new DecimalFormat("#00.00");
	private static DecimalFormat df = new DecimalFormat("#00.0000");
	private static DecimalFormat signFormate = new DecimalFormat("+#,##00.00;-#");
	...
	public static StringBuilder print(Strategy strategy) throws Exception {
		...
		for (StrategyModel strategyModel : strategyModels) {
			...
			double pl = (strategyModel.getLtp() - strategyModel.getAvgPrice()) * strategyModel.getQuantity() * 1000;
			totalPL = totalPL + pl;
			...
			result.append(type + "   " + addZero(strategyModel.getExpiry()) + "   " + strike.format(strategyModel.getStrike()) + "   " + signFormate.format(strategyModel.getQuantity()) + "   "
		...
		result.append(line + "\n");
		result.append("Points :                            [" + df.format(totalPremiumRecived) + "]   [" + df.format(currentPremiumRecived) + "]    ["
				+ df.format(currentPremiumRecived - totalPremiumRecived) + "]");
		result.append("\n" + line + "\n");
		result.append("Total PL : [" + strike.format(totalPL) + "],    At Spot: [" + strategy.getUnderlying_ltp() + "] moved : [" + (diff) + "] - " + strategy.getDataUpdatedAt() + "\n");
		result.append("Min PL " + " :  [" + strike.format(strategy.getTotalPLMin()) + "],    At Spot: [" + strategy.getTotalPLMinSpot() + "] moved : ["
				+ df.format(strategy.getTotalPLMinSpot() - strategy.getTradeSpotPrice()) + "] - " + strategy.getTotalPLMinTime() + "\n");
		result.append("Max PL " + " :  [" + strike.format(strategy.getTotalPLMax()) + "],    At Spot: [" + strategy.getTotalPLMaxSpot() + "] moved : ["
